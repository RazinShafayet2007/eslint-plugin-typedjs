module.exports = {
  meta: {
    type: "problem",
    docs: { 
      description: "Comprehensive type validation for TypedJS",
    ...
    messages: {
      ...
      invalidTuple: "Tuple has incorrect length or element types"
    },
    schema: []
  },
  ...
  create(context) {
    ...
    // Parse TypeScript AST node to type representation
    function parseTypeNode(node) {
      ...
      switch (node.type) {
        ...
        case 'TSTupleType':
          return { 
            kind: 'tuple', 
            elements: node.elementTypes.map(el => {
              if (el.type === 'TSNamedTupleMember') {
                return {
                  kind: 'labeled',
                  label: el.label?.name,
                  optional: !!el.optional,
                  type: parseTypeNode(el.elementType)
                };
              }
              if (el.type === 'TSRestType') {
                return { kind: 'rest', type: parseTypeNode(el.typeAnnotation) };
              }
              if (el.type === 'TSOptionalType') {
                return { kind: 'optionalElement', type: parseTypeNode(el.typeAnnotation) };
              }
              return parseTypeNode(el);
            })
          };

        // Type references (interfaces, type aliases, generics)
        case 'TSTypeReference':
          ...
          if (typeName === 'ReadonlyArray' && node.typeParameters?.params?.length > 0) {
            return { kind: 'readonlyArray', elementType: parseTypeNode(node.typeParameters.params[0]) };
          }
          ...
          if (typeName === 'Promise' && node.typeParameters?.params?.length > 0) {
            return { kind: 'promise', resolveType: parseTypeNode(node.typeParameters.params[0]) };
          }

          // Utility types
          if (typeName === 'Partial' && node.typeParameters?.params?.length > 0) {
            return { kind: 'partial', type: parseTypeNode(node.typeParameters.params[0]) };
          }
          if (typeName === 'Required' && node.typeParameters?.params?.length > 0) {
            return { kind: 'required', type: parseTypeNode(node.typeParameters.params[0]) };
          }
          if (typeName === 'Readonly' && node.typeParameters?.params?.length > 0) {
            return { kind: 'readonly', type: parseTypeNode(node.typeParameters.params[0]) };
          }
          if (typeName === 'Pick' && node.typeParameters?.params?.length === 2) {
            return { 
              kind: 'pick', 
              type: parseTypeNode(node.typeParameters.params[0]),
              keys: parseTypeNode(node.typeParameters.params[1])
            };
          }
          if (typeName === 'Omit' && node.typeParameters?.params?.length === 2) {
            return { 
              kind: 'omit', 
              type: parseTypeNode(node.typeParameters.params[0]),
              keys: parseTypeNode(node.typeParameters.params[1])
    ...
    function matchesType(valueNode, type) {
      ...
      if (type?.kind === 'literal') {
        return valueNode.type === 'Literal' && valueNode.value === type.value;
      }
